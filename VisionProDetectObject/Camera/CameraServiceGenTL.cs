...
namespace Camera
{
    public class CameraServiceGenTL : ICamera
    {
        // private static readonly Lazy<CameraServiceGenTL> lazy =
        //new Lazy<CameraServiceGenTL>(() => new CameraServiceGenTL());

        // public static CameraServiceGenTL Instance { get { return lazy.Value; } }
        ...
        public ICogImage acquiredImage { get; set; }
        public ICogFrameGrabber Camera { get ; set ; }
        public string Formart { get ; set; }

        public CogFrameGrabberGenTLs enumeratedCameras;//CogAcqFifoTool

        public delegate void Notify();
        public event Notify AcqImageCompleted;
        public ICogAcqFifo activeFifo;
        public CogStringCollection availableFormats = null;

        private int currentAcquisitionTicket = -1;
        public CameraServiceGenTL()
        {
            InitializerCamera();
        }
        ...
        public ICogFrameGrabber LoadCamera(string serinumber)
        {
            foreach (ICogFrameGrabber cogFrameGrabber in enumeratedCameras)
            {
                if(cogFrameGrabber.SerialNumber == serinumber && cogFrameGrabber.OwnedGenTLAccess.TLType.Equals("U3V")) 
                    return cogFrameGrabber; 
            }
            return null;
        }
        public string LoadVideoFormat(int index)
        {
            var format = "";
            if (availableFormats!=null)
               format= availableFormats.Count >  0 ?  availableFormats[index] : "";
            return format;
        }
       public string LoadVideoFormat(string s)
        {
        ...
        public void ActiveFifo_Complete(object sender, CogCompleteEventArgs e)
        {
        ...
        public void CreateAcqFifo(ICogFrameGrabber acqFifo,string videoFormat )
        {
            try
            {
                if ( activeFifo == null || videoFormat == null ) 
                    return;
                activeFifo =
                acqFifo.CreateAcqFifo(videoFormat, /* Video Format is what determines what will be set for what the Genicam spec calls the Pixel Format */
                ...
                activeFifo.OwnedExposureParams.Exposure = Exposure;
                activeFifo.OwnedBrightnessParams.Brightness=Brightness;
                activeFifo.OwnedContrastParams.Contrast=Contrast;
            }
            catch
            {
